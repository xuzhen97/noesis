...
export class AliyunDriveOpenApiClient {
	...
	async getDriveInfo(): Promise<{ driveId: string; raw: unknown }> {
		...
		if (!driveId) {
			throw new Error("Aliyun Drive response did not include drive id");
		}
	...
	async ensureFolderPath(input: {
		...
		for (const segment of segments) {
			...
			if (matched) {
				...
				if (!parentFileId) {
					throw new Error(`Aliyun folder ${segment} is missing file_id`);
				}
				continue;
			}
			...
			if (!parentFileId) {
				throw new Error(
					`Aliyun create folder ${segment} did not return file_id`,
				);
			}
		}
		return parentFileId;
