...
/** 生成分片序号列表。 */
export function buildPartInfoList(
	fileSize: number,
	partSize = resolveAliyunPartSize(fileSize),
): Array<{ part_number: number }> {
	if (!Number.isInteger(fileSize) || fileSize < 0) {
		throw new Error("fileSize must be a non-negative integer");
	}
	if (!Number.isInteger(partSize) || partSize < MIN_ALIYUN_PART_SIZE) {
		throw new Error("partSize is too small");
	}
...
/** 单片实际字节数。 */
export function resolvePartSize(
	fileSize: number,
	partSize: number,
	partNumber: number,
): number {
